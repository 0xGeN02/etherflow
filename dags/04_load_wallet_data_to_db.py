...
class WalletBalance(Base):
    """Modelo para balance de wallets"""
...
class WalletTransaction(Base):
    """Modelo para transacciones de wallets"""
...
class WalletAnalysis(Base):
    """Modelo para análisis de wallets"""
...
def load_wallet_balances(**context):
    ...
    if not formatted_files:
        print("No hay archivos formateados para cargar")
        return []
    ...
    try:
        for file in formatted_files:
            try:
                ...
                errors.append({'file': str(file), 'error': str(e)})
                print(f"✗ Error cargando transacciones de {file}: {e}")
...
def load_wallet_analysis(**context):
    ...
    if not analyzed_files:
        print("No hay archivos de análisis para cargar")
        return []
    ...
    try:
        for file in analyzed_files:
            try:
                ...
                if wallet_analysis:
                    # Actualizar existente
                    wallet_analysis.analyzed_at = datetime.fromisoformat(
                ...
                wallet_analysis.analysis_data = json.dumps(analysis)
                ...
                errors.append({'file': str(file), 'error': str(e)})
                print(f"✗ Error cargando análisis de {file}: {e}")
...
with DAG(
    ...
    create_tables_task >> [load_balances_task, load_transactions_task, load_analysis_task]
